...
**Decision: Apache License 2.0 for the simulation's source code** (`index.html`, and the harness/tooling scripts referenced throughout this document), **effective v4.8**. Apache 2.0 was chosen over MIT specifically for its explicit patent grant and patent-retaliation clause — a defensive feature with some relevance if this framework's mechanisms were ever built on commercially, and otherwise just as permissive and widely understood as MIT. The full license text is in `LICENSE-CODE.txt`. **Papers, this document, and the replication page's prose are unaffected — still CC BY 4.0.** The DOI-archived snapshot at `10.17605/OSF.IO/QWTE2` predates this change and its own OSF metadata still reads CC BY 4.0; that isn't retroactively altered by this document, and would need Duke to update it directly at OSF if desired. See the updated footer/citation text in `index.html` and the replication page for how the split is now disclosed to readers.
...
The DOI-archived snapshot at `10.17605/OSF.IO/QWTE2` predates this split and its own OSF-hosted metadata still reads CC BY 4.0 for the whole archived bundle; this document doesn't retroactively change that record — updating it, if desired, is a separate step for Duke to take directly at OSF.
