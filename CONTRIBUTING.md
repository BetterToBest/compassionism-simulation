...
**v4.2 large-N reconfirmation — complete (Aug 2026).** Unlike v4.1, which changed no simulation mechanics and could reuse the v4.0 large-N figures unchanged, v4.2's population-pairing fix touches `makeAgent()` directly, so a full N=5,000 study was run rather than assumed (same methodology as the v4.0/v4.1 studies — a Node.js harness driving `makeLatentPopulation()`/`instantiateAgent()`/`runYear()`/`calcMetrics()`/`bleiMetrics()`/`structuralStability()` directly, seeds 1–5,000, 500 agents, 20 years, shock disabled to match the Full Integration reference preset; script available on request via a GitHub issue, same as the v4.0 large-N study). Headline result: figures across all three scenarios land within a few hundredths of a percentage point of the v4.0/v4.1 numbers they replace (Full Integration Threshold Rate 92.1%, BLEI Poverty 7.9%, EDC-adjusted Gini 0.4843, Median Wealth $81,597; Baseline just as tight) — the population-pairing fix changes which agents get compared, not the model's aggregate behavior. System Stability now reports the new structural metric (Full Integration 94.7%, CCO-Only 95.3%, Baseline 80.6% — not comparable to the old heuristic), and the near-poverty cohort's median time to the Stable tier shifted modestly to 158.8 months / 13.2 years (was ~165 months / 13.7 years) — a genuine, small shift, not noise. Full figures and methodology are in the replication document's Performance Comparison and Methodology Overview, including the near-poverty cohort's Threshold-crossing refinement (now measured as immediate/month-0 rather than "~12 months" — a granularity fix, not a changed finding). The v4.1 40-year cohort-Flourishing extension (N=1,000) was deliberately **not** re-run — it remains a v4.1-mechanics result pending its own reconfirmation pass.

**A methodology finding for anyone reproducing this study.** Running "CCO-Only" as its own top-level scenario using `PRESETS.cco`'s own 75% participation figure gives numbers that don't match this page's documented CCO-Only figures — off by 1–2 percentage points across poverty, wealth, and Gini, consistently in the direction lower participation would produce. The actual large-N methodology's "CCO-Only" figures use 78% participation, matching the Full Integration reference preset — because the interactive tool's own internal CCO-Only comparison (`pCCO` inside `simulate()`) inherits `partRate` from whichever scenario it's being compared against, not from `PRESETS.cco`'s own standalone value. Confirmed by cross-checking against Traditional Welfare Baseline, whose parameters are fully hardcoded and matched immediately either way, ruling out a broader problem. Worth knowing before anyone else starts a reproduction from the preset definitions directly.

**A reproducibility caveat, also for future reproducers.** A single seed-42 run through this session's harness didn't reproduce the exact seed-42 table above bit-for-bit — it landed within about 1–3% on every metric, well inside normal seed-to-seed sampling noise at n=500, but not identical. Aggregate statistics from the same harness at N=200+ matched the documented N=5,000 figures almost exactly (e.g. Threshold Rate 92.19% vs. 92.1% documented), which is strong evidence the harness itself is correct — the likely explanation is that `octaveShape`'s `beta(2,5)` draw uses rejection sampling with `Math.tan`/`Math.exp`/`Math.log`, whose last-bit rounding isn't guaranteed identical across JS engine versions or platforms, and a tiny difference there can cascade into a different (but equally valid) population for one specific seed. Doesn't affect this study's validity — large-N results are inherently robust to single-seed variation — but "seeded RNG produces identical output" (Code Contributions, below) is a same-environment guarantee, not necessarily a cross-environment one, for anything that touches rejection sampling.
