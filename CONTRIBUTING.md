...
- **Code license — CC BY 4.0 is unconventional for software, not just for papers (raised independently by both v4.7 external reviews).** Creative Commons' own guidance recommends against using its licenses for software specifically, since they don't address source-code distribution terms or patent rights the way OSI-approved licenses (MIT, Apache-2.0) do. Both reviews suggested the same split: keep CC BY 4.0 for the papers/prose, move the JavaScript to an OSI license. There's real precedent for projects splitting licenses this way between code and documentation. Not changed in v4.7 — this is a genuine governance/legal decision for this project's authors, not something to change unilaterally alongside a hygiene pass, and mixing licenses across a project this tightly coupled (code, papers, and interactive tool sharing one identity) has its own trade-offs worth thinking through rather than defaulting to the "obvious" answer.
...
- Occupation-*stratified* (not just bimodal) automationRisk — **the transcription barrier may be gone; needs direct verification, not more searching.** v4.6 scoped this to "transcribe the PDF appendix + build a BLS OES crosswalk." Two independent v4.7 searches (this document's own, and a second external ChatGPT pass) both found evidence that a machine-readable version of the full 702-occupation Frey & Osborne table already exists — at least one Kaggle-hosted dataset claiming complete SOC-code coverage, and at least one existing chart (via Datawrapper/New America) that already joins F&O probabilities to BLS OES employment shares, which is exactly the crosswalk piece v4.6 flagged as the remaining work. **Not verified this pass**: whether the Kaggle dataset is actually complete and faithful to the original 702 rows, and what license it carries (Kaggle license terms vary per dataset and need checking before anything is built on top of it) — a second ChatGPT pass also cited a specific "FOWIGS pandemic dataset" as an existing F&O/OES crosswalk, but a direct search turned up no trace of it, so that specific citation is not to be relied on. Good candidate for a contributor to (a) actually pull the Kaggle dataset and spot-check a sample of rows against the original paper's appendix (pp. 57–72) for fidelity, (b) confirm its license permits use here, then (c) decide how agents map to occupations (draw one per agent weighted by BLS employment share, assign that occupation's F&O probability as `automationRisk`) — likely much less transcription work than v4.6 assumed, if the dataset holds up.
- **`WEALTH_FLOOR`/`TARGET_*` sensitivity sweep** — same treatment as the existing λ Calibration Status sweep (below): run N seeds at several candidate `WEALTH_FLOOR` values, report Baseline/Full Integration wealth/Gini/poverty at each, without changing the shipped default. Purely a data-gathering exercise to give the framework's authors something concrete to decide `WEALTH_FLOOR`'s reconsideration (above) against. **Blocked on a validated harness** — this document's large-N studies are run through "a Node.js harness driving the shipped engine functions directly," referenced throughout this file as available on request; reusing that already-validated harness is strongly preferred over a fresh reimplementation, to avoid introducing unverified-model-output risk into a project whose whole audit process exists to catch exactly that.
...
*Better To Best Research Hub · Compassionism Framework Simulation v4.7*  
